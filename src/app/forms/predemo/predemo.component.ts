...
export class PredemoComponent implements OnInit {
	...
	constructor(private fb: FormBuilder, private route: ActivatedRoute, private formService: FormsService, private renderer2: Renderer2) {
	...
	confirmDemo(): void {
		...
		this.formService.savePredemoForm(this.meetingId, formData).subscribe((res) => {
			console.log(res);
	...
	getCookie(name) {
		...
		if (cookieValues.length != 0) return cookieValues[0];
		return undefined;
	}
