...
export class RoomsSearchComponent implements OnInit, OnDestroy {
	...
	constructor(private locationService: LocationsService, private sanitizer: DomSanitizer, private shortcutsService: KeyboardShortcutsService) {}

	ngOnInit() {
		this.locationService
			.getLocationsWithConfigRequest('v1/locations?limit=1000&starred=false')
		...
		this.shortcutsService.onPressKeyEvent$.pipe(takeUntil(this.destroy$), pluck('key')).subscribe((key) => {
			if (key[0] === 'enter') {
	...
	isSelected(room) {
		return (
			findIndex(this.selectedRooms, (selectedRoom) => {
	...
	onSearch(search) {
		...
		if (!search) {
			...
			this.locationService.searchLocations(100, `&search=${search}&starred=false`).subscribe((res) => {
