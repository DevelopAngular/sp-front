...
export class EncounterGroupDescriptionComponent implements OnInit {
  ...
  description(encounter: PreventEncounters): SafeHtml {
    return this.sanitizer.bypassSecurityTrustHtml(`<span style="word-break: break-word">${encounter.conflict_pass_student_name} was going from <span style="text-decoration: underline">${encounter.conflict_pass_origin}</span>
    ...
    from ${this.getTime(encounter.conflict_pass_start_time)} to ${this.getTime(encounter.conflict_pass_end)}. At ${this.getTime(encounter.pass_time)}, ${encounter.conflict_pass_staff_name} attempted to create a pass from
