...
export class EncounterGroupDescriptionComponent implements OnInit {
	...
	description(encounter: PreventEncounters): SafeHtml {
		return this.sanitizer.bypassSecurityTrustHtml(`<span style="word-break: break-word; user-select: text">${
			encounter.conflict_pass_student_name
		} was going from <span style="text-decoration: underline">${encounter.conflict_pass_origin}</span>
    ...
    from ${this.getTime(encounter.conflict_pass_start_time)} to ${this.getTime(encounter.conflict_pass_end)}. At ${this.getTime(
			encounter.pass_time
		)}, ${encounter.conflict_pass_issuer_name || encounter.first_name + encounter.last_name} attempted to create a pass
    <span>${encounter.conflict_pass_issuer_is_staff ? 'for ' + this.group.users[1].first_name : ''}</span>
    from
    <span style="text-decoration: underline">${encounter.origin}</span> to <span style="text-decoration: underline">${
			encounter.destination
		}</span>, but it was prevented. </span>`);
	}
}
