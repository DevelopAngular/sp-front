...
export class EncounterGroupDescriptionComponent implements OnInit {
  ...
  description(encounter: PreventEncounters): SafeHtml {
    ...
    from ${this.getTime(encounter.conflict_pass_start_time)} to ${this.getTime(encounter.conflict_pass_end)}. At ${this.getTime(encounter.pass_time)}, ${encounter.conflict_pass_issuer_name || (encounter.first_name + encounter.last_name)} attempted to create a pass
