			...
			fromEvent(this.selectedFile.nativeElement, 'change')
				.pipe(
					...
					switchMap((_rooms: any[]): Observable<any[]> => {
						return this.userService.getUsersList('_profile_teacher').pipe(
							map((teachers: any[]) => {
			...
			.subscribe(
				...
				(error) => {
					this.toastService.openToast({
						...
						subtitle: 'Please try refreshing the page. If the issue keeps occuring, contact us at support@smartpass.app.',
						type: 'error',
					});
				}
			);
