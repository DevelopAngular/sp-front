...
export class VisibilityRoomComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	constructor(
		...
		this.dialogIds = [this.PANEL_ID, this.GRADE_LEVEl_DIALOG_ID, this.STUDENT_LIST_DIALOG_ID];
	}
	...
	ngOnInit(): void {
		...
		this.grades$ = this.dataService.getGradesList().pipe(takeUntil(this.destroy$), shareReplay(1));
		...
		this.dirty.pipe(takeUntil(this.destroy$), startWith(hasChanged)).subscribe((v: boolean) => {
	...
	openGradeLevelDialog() {
		...
		this.gradeLevelDialog.afterOpened().subscribe(() => {
			...
			const whenLoadedAdjust$ = this.grades$.pipe(
				take(1),
				tap(() => this.adjustGradeDialogScroll())
			);
