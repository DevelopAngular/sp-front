...
export class GridTableDataSource extends DataSource<any> {
	...
	constructor(private initialData$: Observable<any[]>, private viewport: CdkVirtualScrollViewport, private itemSize: number) {
		super();
...
export class SpDataTableComponent implements OnInit, OnDestroy, OnChanges {
	...
	private doNotTranslateTheseColumns: Record<string, Array<string>> = {
		...
		_profile_parent: ['Name', 'Email', 'Students'],
	};
	...
	ngOnInit() {
		...
		this.fakeTemplate = this.domSanitizer.bypassSecurityTrustHtml(
			`<div class="fake-container-block"
                style="margin-left: ${this.currentPage === 'pass_search' || 'encounter_detection' ? '10%' : 'auto'}; width: ${
				this.currentPage === 'pass_search' || 'encounter_detection' ? '90%' : '100%'
			}"
		...
		this.dataSource.loadedData$
			.pipe(
				...
				switchMap((value) => {
					...
					this.columnsToDisplay = this.storage.getItem(this.currentPage)
						? [
								this.displayedColumns[0],
								...this.displayedColumns.slice(1).filter((col) => {
		...
		this.tableService.updateTableColumns$.pipe(withLatestFrom(this.isCheckbox), takeUntil(this.destroy$)).subscribe(([columns, isCheckbox]) => {
			this.columnsToDisplay = isCheckbox ? ['select', this.displayedColumns[0], ...columns] : [this.displayedColumns[0], ...columns];
			this.cdr.detectChanges();
		});

		this.tableService.clearSelectedUsers.pipe(takeUntil(this.destroy$)).subscribe(() => {
			this.selectedObjects = {};
			this.cdr.detectChanges();
		});
	}
