...
export class DashboardComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		this.adminService
			.getYearInReviewData()
			.pipe(tap((d) => console.log('pdf url: ' + d.pdf_url)))
			.subscribe();
	}

	showStartPage(progress: { [id: string]: Onboard }): boolean {
		return (
			(progress && progress['2.landing:first_room'] && !progress['2.landing:first_room'].done) ||
			(progress['2.landing:first_account'] && !progress['2.landing:first_account'].done)
		);
	}
