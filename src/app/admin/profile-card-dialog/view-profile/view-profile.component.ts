...
export class ViewProfileComponent implements OnInit {
	...
	ngOnInit() {
		...
		if (this.data.profile) {
			...
			if ((this.user.isAdmin() || this.user.isAssistant()) && this.user.isTeacher()) {
				this.locationService.getLocationsWithTeacherRequest(this.user).subscribe((locs) => {
			...
			if (this.user.isAssistant()) {
				if (this.data.allAccounts) {
					this.userService.getRepresentedUsers(this.profile._originalUserProfile.id).subscribe((res: any[]) => {
	...
	buildPermissions() {
		...
		const controls = {};
		this.profilePermissions.teacher
			.concat([...this.profilePermissions.admin, ...this.profilePermissions.assistant, ...this.profilePermissions.student])
			.forEach((perm) => {
	...
	openEditAvatar(event) {
		...
		ED.afterClosed()
			.pipe(
				...
				switchMap(() => {
					return this.userService.currentUpdatedAccount$[this.data.role].pipe(filter((res) => !!res));
				}),
				tap((user) => {
	...
	deleteAvatar() {
		this.loadingProfilePicture.next(true);
		this.userService
			.deleteProfilePicture(this.user, this.data.role)
			.pipe(
				filter((res) => !!res),
				take(1)
			)
