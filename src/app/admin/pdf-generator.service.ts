...
export class PdfGeneratorService {
	...
	private getAssests() {
		...
		return zip(
			this.httpService.get(logoPath, { responseType: 'blob' }).pipe(
				tap((src) => {
			...
			this.httpService.get(reportPath, { responseType: 'blob' }).pipe(
				tap((src) => {
			...
			this.httpService.get(ArrowPath, { responseType: 'blob' }).pipe(
				tap((src) => {
			...
			this.httpService.get(ArrowDoublePath, { responseType: 'blob' }).pipe(
				tap((src) => {
	...
	generateProfileInstruction(role: string) {
		this.prepareBaseTemplate('p').subscribe((res) => {
			const doc = res;
	...
	generateReport(data: any, orientation: string = 'p', page: string = '', title?: string, csvName: string = 'TestCSV'): Observable<any> {
		...
		return this.prepareBaseTemplate(orientation).pipe(
			tap((res) => {
				// console.log(this.A4);
				...
				const linkConsumer = (pdfLink) => {
					...
					// One more marameter has been added to pass the raw data so that the user could download an Xlsx file from the dialog as well.
					if (page !== 'explore') {
						LinkGeneratedDialogComponent.createDialog(
							this.dialog,
							'Report Generated Successfully',
							pdfLink,
							page !== 'hallmonitor' ? data : null,
							csvName
						);
					}
				};
