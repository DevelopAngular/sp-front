			...
			const requests$ = this.selectedUsers.map((user) => {
				if (user.isAdmin() && user.isTeacher()) {
					return this.userService
						...
						.pipe(switchMap(() => this.userService.createUserRolesRequest(user, this.permissionsForm.value, '_profile_teacher')));
				} else if (user.isAdmin()) {
					return this.userService.createUserRolesRequest(user, this.permissionsForm.value, '_profile_admin');
				} else if (user.isTeacher()) {
					return this.userService.createUserRolesRequest(user, this.permissionsForm.value, '_profile_teacher');
				} else if (user.isAssistant()) {
					return this.userService.createUserRolesRequest(user, this.permissionsForm.value, '_profile_assistant');
				}
			});
