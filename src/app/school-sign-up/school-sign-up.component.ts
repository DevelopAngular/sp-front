...
export class SchoolSignUpComponent implements OnInit, AfterViewInit {
		...
		this.route.queryParams
			.pipe(
				switchMap((qp: QueryParams) => {
					...
					return this.http
						.get(environment.schoolOnboardApiRoot + `/onboard/schools/check_auth`, {
		...
		this.schoolForm = new FormGroup({
			...
			email: new FormControl(
				...
				[
					...
					Validators.email,
					(fc: FormControl) => {
						return fc.value.indexOf('@') >= 0 && INVALID_DOMAINS.includes(fc.value.slice(fc.value.indexOf('@') + 1).toLowerCase())
							? { invalid_email: true }
							: null;
					},
				],
	...
	createSchool() {
		...
		this.http
			.post(
				environment.schoolOnboardApiRoot + '/onboard/schools',
				{
	...
	checkSchool(school: any) {
		if (school) {
			this.pending.next(true);
			this.http
				.get(constructUrl(environment.schoolOnboardApiRoot + '/onboard/schools/check_school', { place_id: school.place_id }), {
					headers: {
						Authorization: 'Bearer ' + this.AuthToken,
					},
				})
