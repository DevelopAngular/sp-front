...
export class ExclusionGroupsEffects {
	...
	createExclusionGroup$ = createEffect(() => {
		return this.actions$.pipe(
			...
			switchMap((action: any) => {
				return this.encounterPreventionService.createExclusionGroup(action.groupData).pipe(
	...
	updateExclusionGroup$ = createEffect(() => {
		return this.actions$.pipe(
			...
			switchMap((action: any) => {
				return this.encounterPreventionService.updateExclusionGroup(action.group, action.updateFields).pipe(
	...
	removeExclusionGroup$ = createEffect(() => {
		return this.actions$.pipe(
			...
			switchMap((action) => {
				return this.encounterPreventionService.deleteExclusionGroup(action.group.id).pipe(
					switchMap(() => {
						return [
	...
	constructor(private actions$: Actions, private encounterPreventionService: EncounterPreventionService) {}
}
