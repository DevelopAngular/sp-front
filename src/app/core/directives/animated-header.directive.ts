...
export class AnimatedHeaderDirective implements AfterViewInit, OnInit, OnDestroy {
	...
	get headerTopPos() {
		return this.toggleSchoolBarRef
			? (this.topPosition = this.topPosition + this.toggleSchoolBarRef.nativeElement.getBoundingClientRect().height)
			: this.topPosition;
	}
	...
	ngAfterViewInit(): void {
		combineLatest([
			...
			this.http.schoolsLength$.pipe(
				filter((res) => !!res),
				switchMap((schoolsLength) => {
	...
	getsStopPoint() {
		return this.toggleSchoolBarRef
			? this.getHalfHeaderPos() + this.toggleSchoolBarRef.nativeElement.getBoundingClientRect().height
			: this.getHalfHeaderPos();
	}
