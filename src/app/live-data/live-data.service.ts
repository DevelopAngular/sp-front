...
export class LiveDataService {
	...
	private watch<ModelType extends BaseModel, ExternalEventType>(config: WatchData<ModelType, ExternalEventType>): Observable<ModelType[]> {
		 ...
		 */
		const events: Observable<Action<ModelType, ExternalEventType>> = merge(wrappedExternalEvents, passEvents, loopbackEvents, of<'reload'>('reload'));
	...
	watchHallPassesFromLocation(sortingEvents: Observable<HallPassFilter>, filter: Location[], date: Date = null): Observable<HallPass[]> {
		...
		const filters: FilterFunc<HallPass>[] = [makeSchoolFilter(this.http), (pass) => filterIds.indexOf(pass.origin.id) >= 0];
		...
		return this.watch<HallPass, HallPassFilter>({
			...
			handlePollingEvent: makePollingEventHandler([
				new AddItem(
					['hall_pass.create', 'hall_pass.update', 'hall_pass.end', 'pass_request.accept', 'pass_invitation.accept'],
					HallPass.fromJSON,
	...
	watchHallPassesToLocation(sortingEvents: Observable<HallPassFilter>, filter: Location[], date: Date = null): Observable<HallPass[]> {
		...
		const filters: FilterFunc<HallPass>[] = [makeSchoolFilter(this.http), (pass) => filterIds.indexOf(pass.destination.id) >= 0];
	...
	watchFutureHallPasses(filter?: PassFilterType): Observable<HallPass[]> {
		...
		const filters: FilterFunc<HallPass>[] = [makeSchoolFilter(this.http), (pass) => pass.start_time > this.timeService.nowDate()];
		...
		return this.watch<HallPass, string>({
			...
			handlePollingEvent: makePollingEventHandler([
				new AddItem(
					['hall_pass.create', 'hall_pass.update', 'pass_request.accept', 'pass_invitation.accept'],
					HallPass.fromJSON,
					mergeFilters(filters)
				),
				new RemoveItem(['hall_pass.start', 'hall_pass.cancel'], HallPass.fromJSON),
	...
	watchPastHallPasses(filter?: PassFilterType, limit = 50, timeFilter?: string, offset?: string): Observable<HallPass[]> {
		let queryFilter = '';
		const filters: FilterFunc<HallPass>[] = [makeSchoolFilter(this.http)];
		...
		return this.watch<HallPass, string>({
			...
			decoder: (data) => HallPass.fromJSON(data),
			handleExternalEvent: (s: State<HallPass>, e: string) => s,
			handlePollingEvent: makePollingEventHandler([new AddItem(['hall_pass.end'], HallPass.fromJSON, mergeFilters(filters))]),
			handlePost: (s: State<HallPass>) => {
				s.sort = '-start_time';
				return filterHallPasses(s);
			},
	...
	watchInboxRequests(filter: User): Observable<Request[]> {
		...
		const filters: FilterFunc<Request>[] = [makeSchoolFilter(this.http)];
	...
	watchInboxInvitations(filter: User): Observable<Invitation[]> {
		...
		const filters: FilterFunc<Invitation>[] = [makeSchoolFilter(this.http)];
