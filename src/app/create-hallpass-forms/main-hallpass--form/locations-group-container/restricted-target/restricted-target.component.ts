...
export class RestrictedTargetComponent implements OnInit {
	...
	constructor(private formService: CreateFormService, public sanitizer: DomSanitizer) {}

	get headerGradient() {
		const colors = this.formState.data.direction.pinnable?.color_profile.gradient_color;
		return colors ? 'radial-gradient(circle at 98% 97%,' + colors + ')' : '';
	}

	get localSearch() {
		return (
			(this.formState.forLater && this.formState.data.direction.to?.scheduling_request_mode === 'teacher_in_room') ||
