...
export class GroupsStep3Component implements OnInit, AfterViewInit, OnDestroy {
	...
	constructor(private http: HttpService, private userService: UserService, private changeDetectionRef: ChangeDetectorRef) {}
	...
	ngOnInit() {
		...
		this.form.valueChanges.pipe(takeUntil(this.destroy$), skip(1)).subscribe((val: any) => {
		...
		fromEvent(this.studentEmailsFile.nativeElement, 'change')
			.pipe(
				...
				switchMap((_emails: string[]): Observable<any> => {
					...
					return this.userService.getUsersList('_profile_student').pipe(
						map((students: User[]) => {
							const result = {
								existingStudents: [],
								unknown: [],
	...
	updateGroup() {
		...
		if (dto.users.length) {
			this.userService.updateStudentGroupRequest(this.editGroup.id, dto).subscribe((group: StudentList) => {
	...
	removeGroup() {
		this.userService.deleteStudentGroupRequest(this.editGroup.id).subscribe((group: StudentList) => {
