...
export class MainHallPassFormComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		combineLatest([this.userService.user$.pipe(filter((r) => !!r)), this.userService.effectiveUser])
			.pipe(takeUntil(this.destroy$))
			.subscribe({
				next: ([user, effectiveUser]) => {
					this.user = effectiveUser ? User.fromJSON(effectiveUser.user) : User.fromJSON(user);
